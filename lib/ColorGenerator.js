...
export function getGradationColor(self, colorRange, colorDomain) {
   ...
   * @param {array} colorDomain - Array of minimum, middle and maximum number
   */
  ...
  // colorDomain = colorDomain.filter(Boolean);
  // if (colorRange.length === 2 || colorRange.length === 3) {
  //   domain = colorDomain;
  // }

  console.log(colorDomain.length);
  console.log(domain);
  return d3.scaleLinear().domain(domain).range(colors);
  // return d3
  //   .scaleLinear()
  //   .domain([-50, 0, 100])
  //   .range(["#6590e6", "#ffffff", "#F75976"]);
}
