...
/**
 * @param {HTMLElement} self
 * @param {array} gradationColor
 * @return {function} カラー生成関数
 */
export function getGradationColor(self, gradationColor) {
  let colors = getColorSeries(self);
  gradationColor = gradationColor.filter(Boolean);
  if (gradationColor.length === 2 || gradationColor.length === 3) {
