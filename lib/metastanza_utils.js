...
export default {
  ...
  showApiError: function(targetElement){
    const removeApiError = function(){ d3.select(targetElement).select("#error_message").remove(); }
    d3.select(targetElement).append("p").attr("id", "error_message").text("API error");
