...
// TODO: test
export function sparql_or_fetch(url, sparql = false, stanza_type = "table") {
  ...
  if (sparql) {
    ...
    case "tree":
      return sparql2tree(data)
    case "graph":
      return sparql2graph(data)
    }
  } else {
    return data
  }
...
// e.g. https://static.observableusercontent.com/files/e65374209781891f37dea1e7a6e1c5e020a3009b8aedf113b4c80942018887a1176ad4945cf14444603ff91d3da371b3b0d72419fa8d2ee0f6e815732475d5de
...
export function sparql2graph(json, params) {
  const head = json.head.vars;
  const data = json.results.bindings;
...
export function appendDlButton(buttonDiv, svg, filename, stanza) {
  const dlButtonSVG = d3
    .select(buttonDiv)
    .append("svg")
    .attr("id", "dl_button")
    .attr("width", 32)
  ...
  const downloadImg = function (svg, format, filename, stanza) {
    ...
    const tmp = svg.node().outerHTML.match(/^([^>]+>)(.+)$/);
