...
export default class Legend extends LitElement {
        ...
        <tbody>
          ${this.items.map(item => {
            ...
            <tr data-id="${item.id}">
              ...
              ${item.value ? html`<td class="${(typeof item.value).toLowerCase()}">${item.value}</td>` : ''}
            </tr>`;
          })}
        </tbody>
      </table>
  ...
  setup(items, container, opt) {
    ...
    const containerRect = this.shadowRoot.querySelector('.origin').getBoundingClientRect();
      ...
      this.shadowRoot.querySelectorAll('.legend > table > tbody > tr').forEach(tr => {
        tr.addEventListener('mouseover', () => {
          ...
          if (datum) {
            ...
            leader.style.left = (targetRect.x + targetRect.width * .5 - containerRect.x) + 'px';
            ...
            if (legendMiddle < targetMiddle) {
              ...
              leader.style.top = (legendMiddle - containerRect.y) + 'px';
              leader.style.height = (targetMiddle - legendMiddle) + 'px';
            } else {
              leader.dataset.direction = 'bottom';
              leader.style.top = (targetMiddle - containerRect.y) + 'px';
