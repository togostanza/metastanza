...
export default async function quantileDotPlot(stanza, params) {
  ...
  const scales = [
    {
      name: "x",
      domain: [params["x-domain-start"], params["x-domain-end"]],
      range: "width",
    },
    {
      name: "y",
  ...
  const symbol = {
    ...
    encode: {
      ...
      update: {
        fill: { signal: "datum.bin <" + getSignalByColor('var(--togostanza-fill-selected)','var(--togostanza-fill-unselected)') },
      },
    },
  };
  ...
  const rule = {
    ...
    encode: {
      update: {
        ...
        y: { value: 0 },
        y2: { signal: "height" },
        stroke: { signal: getSignalByColor("var(--togostanza-stroke-signal)") },
      },
    },
  };
  ...
  console.log(params["x-domain-start"]);
