...
export default async function populationPyramid(stanza, params) {
  const vegaJson = await fetch(
    "https://vega.github.io/vega/examples/population-pyramid.vg.json"
  ).then((res) => res.json());
  ...
  const scales = [
    ...
    },
    {
      ...
      type: "ordinal",
      domain: [1, 2],
      range: ["var(--stogozana-group2-color)", "var(--stogozana-group1-color)"],
    },
  ];
  ...
  const groupScales = (num) =>{
    return {
      name: "x",
      type: "linear",
      range: num === 1 ? [{ signal: "chartWidth" }, 0] : [0, {"signal": "chartWidth"}],
      nice: true,
      zero: true,
