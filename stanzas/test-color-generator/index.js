...
export default class VennStanza extends Stanza {
  ...
  async render() {
    ...
    const colorScale = d3.scaleLinear()
      .domain([...Array(colors.length)].map((color, index) =>  numOfGradation / colors.length * index))
      .range(colors);
    ...
    this.root.append(ul);
    ul.innerHTML = [...Array(numOfGradation)].map((_, index) => {
      return `<li style="height: 20px; width: 20px; background: ${colorScale(index)}"></li>`
    }).join('');
