...
export default class Barchart extends Stanza {
  ...
  async render() {
    ...
    const xTickSize = parseInt(this.params["xtick-size"]) || 0;
    const yTickSize = parseInt(this.params["ytick-size"]) || 0;
    const axisTitleFontSize =
      parseInt(css("--togostanza-title-font-size")) || 10;
    const barPaddings = parseFloat(this.params["bar-paddings"]) || 0;
    const barSubPaddings = parseFloat(this.params["bar-sub-paddings"]) || 0;
    ...
    try {
      ...
      const redrawSVG = (
        ...
        const update = (values) => {
          ...
          function updateStackedBars(values) {
            ...
            const dataset = [];
            for (const entry of d3
              .group(values, (d) => d[xKeyName])
              .entries()) {
              dataset.push({
                x: entry[0],
                ...Object.fromEntries(
            ...
            gs.join(
              ...
              .attr("data-html", "true")
              .attr("class", (d) => {
                return `data-${gSubKeyNames.findIndex(
                  (item) => item === d.key
                )}`;
              });
          }
          ...
          legend.addEventListener("click", (e) => {
            ...
            if (parentNode.nodeName === "TR") {
              ...
              const newData = values.filter(
                (item) =>
                  !toggleState.get(
                    "" + gSubKeyNames.indexOf(item[groupKeyName])
                  )
              );
      ...
      console.error("--togostanza-barchart ERROR:", e);
    }
  }
}
