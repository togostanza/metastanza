  ...
  async render() {
    ...
    try {
      ...
      if (!values.some((val) => yKeyName in val || parseFloat(val[yKeyName]))) {
        throw new Error("No y-axis key found in data");
      }
      if (!values.some((val) => xKeyName in val || parseFloat(val[xKeyName]))) {
        throw new Error("No x-axis key found in data");
      }
      ...
      const redrawSVG = (
        ...
        if (showLegend !== "none") {
          this.legend.shadowRoot.addEventListener("click", (e) => {
      ...
      console.error("--togostanza-barchart ERROR:", e.message);
    }
  }
}
