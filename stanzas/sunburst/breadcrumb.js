  ...
  var main = function (selection) {
    selection.each(function scope() {
      ...
      function breadcrumbShow(array, d) {
        ...
        //each arrows wrapper
        var g = breadcrumbTrail.selectAll("g").data(sequenceArray, (d) => d.id);
        g.exit().remove();
        var entering = g.enter().append("svg:g").style("cursor", "pointer");

        //Add title
        entering.append("title").text((d) => d.title);
        ...
        all.each(function (d, i, arr) {
          ...
          wrapper.attr(
            ...
            "translate(" +
              (startX - yDiv * attrs.wrapWidth) +
              "," +
              yDiv * (attrs.height + attrs.marginTop) +
              ")"
          );
          ...
          if (attrs.leftDirection) {
            poligons.attr(
              "points",
              rotatedBreadcrumbPoints(d, i, arr, bbox.width + 35)
            );
          } else {
            poligons.attr(
              "points",
              breadcrumbPoints(d, i, arr, bbox.width + 35)
            );
          }
      ...
      showBreadcrumbs = function () {
        ...
        breadcrumbTrail = attrs.container.patternify({
          tag: "g",
